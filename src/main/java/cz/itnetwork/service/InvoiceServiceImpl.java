...
public class InvoiceServiceImpl implements InvoiceService {
    ...
    public InvoicesPageDTO getPage(InvoiceFilter invoiceFilter) {
        ...
        return invoiceMapper.mapToDTO(pageEntities, invoiceFilter.getLimit());
    }
    ...
    /**
     * <p>Attempts to fetch person's reference.</p>
    ...
    private Page<InvoiceEntity> getPageEntities(InvoiceFilter invoiceFilter) {
        ...
        Sort sortBy = Sort.by(invoiceFilter.getNameOfProperty());
        ...
        Pageable pageableSet = PageRequest.of(invoiceFilter.getPageNumber(), invoiceFilter.getLimit(), sortSet);    // obsahuje jen data požadavku na stránkoování: stránka od, delka, způsob řazení
        final Page<InvoiceEntity> pageEntities = invoiceRepository.findAll(invoiceSpecification, pageableSet);            // zabalená 1 strana dat nesoucí informace o celk. počtu stránek a celk. počtu položek
        final Page<InvoiceEntity> controlledPageEntities;
        if (pageEntities.getTotalPages() < (invoiceFilter.getPageNumber() + 1) && pageEntities.getTotalPages() > 0) { // při jediné stránce bychom skončili s chybou na -1...
